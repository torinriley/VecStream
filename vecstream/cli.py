...
class VecStreamCLI:
    def __init__(self, store_path=None, use_hnsw=True):
        ...
        try:
            ...
            self.default_collection = self.collection_manager.create_collection(
                "default"
            )
    ...
    def load_model(self):
        ...
        if self.model is None:
            rprint("[bold cyan]Loading embedding model...[/bold cyan]")
    ...
    def search(
        self, query_text, collection_name=None, k=5, threshold=0.0, filter_metadata=None
    ):
        ...
        if filter_metadata:
            ...
            if isinstance(filter_metadata, str):
                try:
                    ...
                    rprint(f"[bold red]Error: Invalid filter format.[/bold red]")
                    return []
            ...
            # Use the query engine otherwise
                ...
                all_results = collection.store.search_similar(
                    query_embedding, k=len(collection.store.vectors)
                )
                results = []
            ...
            if collection.use_hnsw and collection.hnsw_index is not None:
                ...
                results = collection.store.search_similar(
                    query_embedding, k=k, threshold=threshold
                )
    ...
    def _get_collection(self, collection_name=None):
        ...
        try:
            ...
            return self.collection_manager.create_collection(
                collection_name, use_hnsw=self.use_hnsw
            )
    ...
    def delete_collection(self, collection_name):
        ...
        if collection_name == "default":
            rprint("[bold red]Cannot delete default collection.[/bold red]")
            return False
...
@click.option(
    "--use-hnsw/--no-hnsw",
    default=True,
    help="Whether to use HNSW indexing for better performance.",
)
@click.pass_context
    ...
    try:
        if metadata:
            try:
                ...
                rprint(f"[bold red]Error: Invalid JSON metadata.[/bold red]")
                return
        else:
        ...
        rprint(
            f"[bold green]Added document with ID '{id}'{collection_info}[/bold green]"
        )
    except Exception as e:
        rprint(f"[bold red]Error adding document: {str(e)}[/bold red]")
    ...
    try:
        ...
        if filter:
            try:
                ...
                rprint(f"[bold red]Error: Invalid JSON filter.[/bold red]")
                return
        ...
        rprint(f"[bold red]Error searching: {str(e)}[/bold red]")
...
def get(ctx, id, collection):
    ...
    try:
        ...
        if metadata is None:
            collection_info = f" in collection '{collection}'" if collection else ""
            rprint(
                f"[yellow]Document with ID '{id}'{collection_info} not found.[/yellow]"
            )
            return
        ...
        rprint(f"[bold red]Error retrieving document: {str(e)}[/bold red]")
...
def remove(ctx, id, collection):
    ...
    try:
        ...
        if success:
            ...
            rprint(
                f"[bold green]Removed document with ID '{id}'{collection_info}[/bold green]"
            )
        else:
            collection_info = f" in collection '{collection}'" if collection else ""
            rprint(
        ...
        rprint(f"[bold red]Error removing document: {str(e)}[/bold red]")
...
@click.option(
    "--collection",
    "-c",
    help="Collection to display info for (omit for all collections).",
)
@click.pass_context
def info(ctx, collection):
    ...
    try:
        ...
        if collection:
            ...
            if not stats:
                rprint(f"[yellow]Collection '{collection}' not found.[/yellow]")
                return
        ...
        # Show list of collections
            ...
            # Display collection list header
            rprint("\n[bold]Available Collections:[/bold]")

            # Display storage path
            rprint(f"\n[bold]Storage Location:[/bold] {cli_instance.store_path}")
    except Exception as e:
        rprint(f"[bold red]Error displaying information: {str(e)}[/bold red]")


@cli.command()
@click.argument("name")
@click.option(
    "--use-hnsw/--no-hnsw", default=True, help="Whether to use HNSW indexing."
)
@click.pass_context
def create_collection(ctx, name, use_hnsw):
    ...
    try:
        collection = cli_instance.collection_manager.create_collection(
            name, use_hnsw=use_hnsw
        )
        rprint(f"[bold green]Created collection '{name}'[/bold green]")
    except ValueError as e:
        rprint(f"[bold red]{str(e)}[/bold red]")
    except Exception as e:
        rprint(f"[bold red]Error creating collection: {str(e)}[/bold red]")
...
def delete_collection(ctx, name):
    ...
    try:
        ...
        if success:
            rprint(f"[bold green]Deleted collection '{name}'[/bold green]")
        else:
            rprint(f"[yellow]Collection '{name}' not found.[/yellow]")
    except Exception as e:
        rprint(f"[bold red]Error deleting collection: {str(e)}[/bold red]")
